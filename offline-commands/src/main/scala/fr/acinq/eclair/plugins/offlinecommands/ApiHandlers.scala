...
object ApiHandlers {
  ...
  def registerRoutes(kit: OfflineCommandsKit, eclairDirectives: EclairDirectives): Route = {
    ...
    val close: Route = postRequest("offlineclose") { implicit t =>
      formFields(channelIdsFormParam, "forceCloseAfterHours".as[Long] ?, "scriptPubKey".as[ByteVector](bytesUnmarshaller).?, "preferredFeerateSatByte".as[FeeratePerByte].?, "minFeerateSatByte".as[FeeratePerByte].?, "maxFeerateSatByte".as[FeeratePerByte].?) {
