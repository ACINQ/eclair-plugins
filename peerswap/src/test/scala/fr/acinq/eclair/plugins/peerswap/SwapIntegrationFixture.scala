...
object SwapIntegrationFixture {
  def swapRegister(node: MinimalNodeFixture): ActorRef[SwapRegister.Command] = {
    ...
    node.system.spawn(Behaviors.supervise(SwapRegister(node.nodeParams, node.paymentInitiator, node.watcher.ref.toTyped, node.register, node.switchboard, node.wallet, keyManager, db, Seq())).onFailure(SupervisorStrategy.stop), s"swap-register-${node.nodeParams.alias}")
  }
