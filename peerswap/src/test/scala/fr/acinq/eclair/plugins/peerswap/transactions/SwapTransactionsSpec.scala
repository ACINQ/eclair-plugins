...
class SwapTransactionsSpec extends TestKitBaseClass with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  def createFixture(): Fixture = {
    ...
    Fixture(watcher, probe)
  }

  case class Fixture(watcher: TestProbe, probe: TestProbe)
