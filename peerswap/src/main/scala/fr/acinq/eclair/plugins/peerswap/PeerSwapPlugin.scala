...
case class PeerSwapKit(nodeParams: NodeParams, system: ActorSystem, swapRegister: ActorRef[SwapRegister.Command], db: SwapsDb) {
  ...
  def swapHistory()(implicit timeout: Timeout): Future[Iterable[SwapData]] =
    runAsync(db.list())
}
